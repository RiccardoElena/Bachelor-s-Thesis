...
Moreover, is possible to isolate a \emph{subgrammar} by considering only the rules that generate atomic formulae (excluding so the ones deriving from \(\phi\)). Doing this easily allows to define the concept of \bold{subterm} as a \emph{proper subtree} of the syntactic tree that represents an atomic formula (or by extension a literal).
...
If we consider multiple atomic formulae (or literals), is possible to represent them as a \bold{forest} of syntactic trees, where each tree represents an atomic formula.
...
A unifier \(\sigma\) for two terms \(\tau_1\) and \(\tau_2\) is called a \bold{most general unifier} (MGU) if it is more general than any other unifier of \(\tau_1\) and \(\tau_2\).
...
The result of Skolemization is not logically equivalent to the original formula, but it is \bold{equisatisfiable}: both formulae are satisfiable in exactly the same set of interpretations.
